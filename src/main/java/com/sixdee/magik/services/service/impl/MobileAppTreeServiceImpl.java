...
/*	@Override
	...
	/*@Override
	public MobileAppTreeDTO createCategory(MobileAppTreeDTO mobileAppTreeDTO) {
		try {
 ...
 ///
	...
	/*@Override
	public MobileAppTreeDTO editCategory(MobileAppTreeDTO mobileAppTreeDTO) {
		// TODO Auto-generated method stub
		try {
...
/*	@Override
	...
	/*@Override
	public MobileAppTreeDTO createMessage(MobileAppTreeDTO mobileAppTreeDTO) {
		// TODO Auto-generated method stub
		try {

			boolean flag = CreateMobileAppMenuName(mobileAppTreeDTO);
			if (flag) {
				String menuId = getMenuId(mobileAppTreeDTO);
	...
	/*@Override
	public MobileAppTreeDTO editMessage(MobileAppTreeDTO mobileAppTreeDTO) {
		// TODO Auto-generated method stub
		try {
	...
	/*private boolean createMobileAppCategory(MobileAppTreeDTO dto, int category_order) {
		boolean flag = false;
		String sql = "";
		Transaction txn;
		Session session;
		try {
			...
			System.out.println("category name:: " + dto.getCampName());
			session = sessionFactory.getCurrentSession();
			txn = session.beginTransaction();
			sql = "INSERT INTO MOBILE_APP_CATEGORY_MASTER(CATEGORY_NAME,STATUS,CATEGORY_ORDER) VALUES ('"
					+ dto.getCampName() + "','T','" + category_order + "')";

			logger.info("createCategoryMobileApp-->" + sql);
			...
			System.out.println("flag in createMobileAppCategory:: " + flag);
	...
	/*public boolean editMobCategory(MobileAppTreeDTO dto) {
		boolean flag = false;
		String sql = "";
		Transaction txn;
		Session session;
		try {
			session = sessionFactory.getCurrentSession();
			txn = session.beginTransaction();
			sql = "UPDATE MOBILE_APP_CATEGORY_MASTER SET CATEGORY_NAME = '" + dto.getCampName()
					+ "' WHERE CATEGORY_ID='" + dto.getCampId() + "'";
			logger.info("editCategoryMobileApp-->" + sql);
		...
		try {
			...
			if (result1 != null && result1.size() > 0) {
				for (Object[] aRow : result1) {
					...
					dto.setMenuName(aRow[3].toString());
					dto.setTitle(aRow[4].toString());
					dto.setDeepLink(aRow[5].toString());
					dto.setCampaignType(aRow[6].toString());
					dto.setPriority(aRow[7].toString());
					dto.setStartDate(aRow[8].toString());
					dto.setEndDate(aRow[9].toString());
	...
	/*private boolean CreateMobileAppMenuName(MobileAppTreeDTO mobileAppTreeDTO) {
		// TODO Auto-generated method stub
		...
		try {
			...
			String sql = "INSERT INTO MOBILE_APP_MENU_DETAILS (MENU_NAME,CAMP_ID,CREATE_USER) VALUES ('"
					+ mobileAppTreeDTO.getMenuName() + "','" + mobileAppTreeDTO.getCampId() + "','"
					+ mobileAppTreeDTO.getUserId() + "')";
			logger.info("[sql] : " + sql);
			Query qry = session.createSQLQuery(sql);
	...
	private boolean CreateMobileAppMessage(MobileAppTreeDTO mobileAppTreeDTO, String menuId) {
		// TODO Auto-generated method stub
		Transaction txn = null;
		boolean flag = false;
		Session session = null;
		...
		try {
			...
			List list = mobileAppTreeDTO.getMessages();
			System.out.println("list Size" + list.size());
			for (int j = 0; j < list.size(); j++) {
				MobileAppLangMessagesTO dto = (MobileAppLangMessagesTO) list.get(j);
				txn = session.beginTransaction();
				System.out.println("get j" + j);

				sql = "INSERT INTO MOBILE_APP_MENU_MSG_DETAILS (MENU_ID,LANGUAGE_ID,MESSAGE,USER_ID,TITLE,USSD_SENDER_ID,APP_API_LINK,WEB_API_LINK,MSG_TYPE) VALUES"
						+ " ('" + menuId + "','" + dto.getLangId() + "','" + getHex(dto.getMessage()) + "','"
						+ mobileAppTreeDTO.getUserId() + "','" + dto.getTitle() + "','" + dto.getUssdSenderId() + "','"
						+ dto.getAppApiLink() + "','" + dto.getWebApiLink() + "','" + dto.getMessageType() + "')";
				logger.info("[sql] : " + sql);
				Query qry = session.createSQLQuery(sql);
				int i = qry.executeUpdate();
			...
			List inboundList = mobileAppTreeDTO.getInboundMessages();
			System.out.println("inboundList Size" + inboundList.size());
			for (int j = 0; j < inboundList.size(); j++) {
				MobileAppLangMessagesTO dto = (MobileAppLangMessagesTO) inboundList.get(j);
				txn = session.beginTransaction();
				System.out.println("get j" + j);

				sql = "INSERT INTO MOBILE_APP_MENU_MSG_DETAILS (MENU_ID,LANGUAGE_ID,MESSAGE,USER_ID,TITLE,MSG_TYPE,DEEP_LINK,CAMPAIGN_TYPE,PRIORITY,START_DATE,END_DATE) VALUES"
						+ " ('" + menuId + "','" + dto.getLangId() + "','" + getHex(dto.getMessage()) + "','"
						+ mobileAppTreeDTO.getUserId() + "','" + dto.getTitle()+ "','"+dto.getMessageType()+"','"
						+ dto.getDeepLink()+"','"+dto.getCampaignType()+"','"+dto.getPriority()+"','"+dto.getStartDate()+"','"+dto.getEndDate()+"')";
				logger.info("[sql] : " + sql);
				Query qry = session.createSQLQuery(sql);
				int i = qry.executeUpdate();
				System.out.println("============ i =============" + i);
	...
	/*private String getMenuId(MobileAppTreeDTO mobileAppTreeDTO) {
		...
		try {
			session = sessionFactory.getCurrentSession();
			sql = "SELECT MENU_ID,MENU_NAME FROM MOBILE_APP_MENU_DETAILS WHERE MENU_NAME = '"
					+ mobileAppTreeDTO.getMenuName() + "'";
			logger.info("[sql] : " + sql);
			Query qry = session.createSQLQuery(sql);
	...
	/*private boolean EditMobileAppMenuName(MobileAppTreeDTO mobileAppTreeDTO) {
		...
		try {
			...
			sql = "UPDATE MOBILE_APP_MENU_DETAILS SET MENU_NAME = '" + mobileAppTreeDTO.getMenuName()
					+ "' WHERE MENU_ID ='" + mobileAppTreeDTO.getMenuId() + "' AND CAMP_ID = '"
					+ mobileAppTreeDTO.getCampId() + "' AND CREATE_USER ='" + mobileAppTreeDTO.getUserId() + "' ";
			logger.info("[sql] : " + sql);
			Query qry = session.createSQLQuery(sql);
			int i = qry.executeUpdate();
			System.out.println("============ i =============" + i);
	...
	private boolean EditMobileAppMessage(MobileAppTreeDTO mobileAppTreeDTO) {
		...
		try {
			...
			for (int j = 0; j < list.size(); j++) {
				...
				MobileAppLangMessagesTO dto = (MobileAppLangMessagesTO) list.get(j);
				sql = "INSERT INTO MOBILE_APP_MENU_MSG_DETAILS (MENU_ID,LANGUAGE_ID,MESSAGE,USER_ID,TITLE,USSD_SENDER_ID,APP_API_LINK,WEB_API_LINK,MSG_TYPE) VALUES" + " ('"
						+ mobileAppTreeDTO.getMenuId() + "','" + dto.getLangId() + "','" + getHex(dto.getMessage())
						+ "','" + mobileAppTreeDTO.getUserId() + "','" + dto.getTitle() + "','" + dto.getUssdSenderId() + "','"
								+ dto.getAppApiLink() + "','" + dto.getWebApiLink() + "','" + dto.getMessageType() + "')";
				logger.info("[sql] : " + sql);
			...
			List list1 = mobileAppTreeDTO.getInboundMessages();
			for (int j = 0; j < list1.size(); j++) {
				txn = session.beginTransaction();
				MobileAppLangMessagesTO dto = (MobileAppLangMessagesTO) list1.get(j);
				sql = "INSERT INTO MOBILE_APP_MENU_MSG_DETAILS (MENU_ID,LANGUAGE_ID,MESSAGE,USER_ID,TITLE,MSG_TYPE) VALUES" + " ('"
						+ mobileAppTreeDTO.getMenuId() + "','" + dto.getLangId() + "','" + getHex(dto.getMessage())
						+ "','" + mobileAppTreeDTO.getUserId() + "','"+dto.getTitle()+"','"+dto.getMessageType()+"')";
				logger.info("[sql] : " + sql);
				Query qry1 = session.createSQLQuery(sql);
